...
export const OccupationTypeValues: OccupationType[] = ['adult', 'student', 'ministry-stipend', 'ministry-salary'];
...
export const calculateTotalPrice = (formData: FormData): {
  ...
  const tshirtTotal = formData.orderTshirt
    ? formData.tshirtOrders.reduce((sum: number, order: { quantity: number }) => sum + (order.quantity * PRICING_CONFIG.tshirtRate), 0)
    : 0;
